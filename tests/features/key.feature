Feature: Enterprise Feed Key
  ...
  Rule: Getting the key status
    ...
    Scenario: If the user is authenticated, a feed key exists but is not readable an error should be returned
      Given a valid feed key exists in the system
      Given the feed key file is not readable
      When I send a GET request to the key status endpoint
      Then the response status code should be 500
      And the response body should be valid JSON
      And the JSON message should be "Internal server error: File error."
