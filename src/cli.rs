...
mod tests {
    ...
    fn try_parse_from(args: Vec<&str>) -> Result<Cli, clap::error::Error> {
        Cli::try_parse_from(std::iter::once("test").chain(args.into_iter()))
    }

    #[test]
    fn test_default_cli() {
        let cli = try_parse_from(vec![]).unwrap();
    ...
    #[test]
    fn test_parse_port() {
        let cli = try_parse_from(vec!["--port", "8080"]).unwrap();
        assert_eq!(cli.port, 8080);
    }

    #[test]
    fn test_parse_server() {
        let cli = try_parse_from(vec!["--server", "0.0.0.0"]).unwrap();
        assert_eq!(cli.server, "0.0.0.0");
    }

    #[test]
    fn test_parse_feed_key_path() {
        let cli = try_parse_from(vec!["--feed-key-path", "/tmp/key"]).unwrap();
        assert_eq!(cli.feed_key_path, "/tmp/key");
    }

    #[test]
    fn test_parse_log() {
        let cli = try_parse_from(vec!["--log", "my_crate=debug"]).unwrap();
        assert_eq!(cli.log, "my_crate=debug");
    }

    #[test]
    fn test_parse_tls_server_cert_without_key() {
        let cli = try_parse_from(vec!["--tls-server-cert", "/tmp/cert.pem"]);
        assert!(cli.is_err());
        assert_eq!(
            cli.err().unwrap().kind(),
    ...
    fn test_parse_tls_server_key_without_cert() {
        let cli = try_parse_from(vec!["--tls-server-key", "/tmp/key.pem"]);
        assert!(cli.is_err());
        assert_eq!(
            cli.err().unwrap().kind(),
            clap::error::ErrorKind::MissingRequiredArgument
    ...
    fn test_parse_tls_client_certs_without_server_cert_and_key() {
        let cli = try_parse_from(vec!["--tls-client-certs", "/tmp/ca.pem"]);
        assert!(cli.is_err());
        assert_eq!(
            cli.err().unwrap().kind(),
            clap::error::ErrorKind::MissingRequiredArgument
