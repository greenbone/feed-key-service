...
pub enum CertificateError {
    ...
    CertificateLoadingError(String, pem::Error),

    #[error("Could not load key from {0}. {1}")]
    KeyLoadingError(String, pem::Error),

    #[error("Could not load certificate chain from {0}. {1}")]
    CertificateChainLoadingError(String, pem::Error),

    #[error("Could not add certificate to root store. {0}")]
    RootCertStoreError(rustls::Error),
}

pub fn load_certificate(cert_path: &str) -> Result<CertificateDer<'static>, CertificateError> {
...
pub fn load_private_key(key_path: &str) -> Result<PrivateKeyDer<'static>, CertificateError> {
